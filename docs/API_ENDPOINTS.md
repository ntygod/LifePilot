...
> 仅在 `lifepilot.gateway.channels.web.enabled=true` 时启用。故意不提供 POST 创建端点——创建入口保持在 LLM 对话中由自然语言触发 cron 工具（`CronActionDispatchExecutor`），本 Controller 只管存量管理（列出、编辑、暂停、删除）。暂停/恢复通过 PUT 更新 `status` 字段实现（active ↔ paused）。
...
`ScheduledTaskResponse` 字段：`{id, name, schedule, instruction, status, skillIds, projectId, createdAt, updatedAt}`。`schedule` 为 Spring 6 位 cron 表达式，`status` 取值 `active` / `paused` / `completed`，`projectId` 为 `null` 表示归属主账户。
