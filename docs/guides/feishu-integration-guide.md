...
如果需要使用交互卡片功能（如工具授权审批卡片），必须在应用配置中开启卡片回调：

1. 进入应用 →「开发配置」→「事件与回调」
2. 找到「卡片回调」部分，开启卡片交互回调
3. 回调方式选择与消息事件一致的模式：
   - **WebSocket 模式**：选择「使用长连接接收」
   - **Webhook 模式**：填写回调 URL（与事件订阅相同）

> **注意**：如果不开启此配置，交互卡片的按钮点击会返回飞书错误码 200080，且服务端不会收到任何回调事件。
...
### 8.3 交互卡片按钮点击报错 200080

- 确认应用的「卡片交互回调」已开启
- 回调方式需与消息事件一致（WebSocket 模式选「使用长连接接收」）
- 如果刚开启，需重新发布应用版本后生效
- 查看 connector 日志是否有 `收到卡片回调事件` 字样，无此日志说明飞书未推送回调

### 8.4 Webhook 模式 challenge 验证失败
