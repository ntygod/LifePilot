...
工具对 LLM 的暴露采用**核心常驻 + 按需发现**模式：

- **核心 pinned 工具**：`memory` / `file.read` / `status` / `skill.load` / `tool.search` 启动即注入 LLM 上下文
- **非核心 Java 原生工具与 MCP 工具**：全部注册进 `DynamicToolRegistry` 并进入 `tool_search_index`，由 `tool.search` 发现后写入 `ReactAgentState.discoveredToolIds`，下一轮再注入完整 schema

`skill.load` 只返回 Skill 指南内容并注入上下文；`suggested_tools` 仅作为 UI / 检索 / 人工参考元数据，不再直接改变工具可见集。
...
graph TB
    subgraph "Agent 引擎"
        ...
        BRIDGE["ToolBridgeAgentToolProvider<br/>core pinned ∪ discovered 过滤"]
    end

    subgraph "工具注册"
        REG["DynamicToolRegistry<br/>动态工具注册表"]
        BUILTIN_REG["BuiltinToolRegistrar<br/>+ ToolValidator 启动校验"]
        META_REG["BuiltinToolSearchProvider<br/>注册 tool.search"]
    end
    ...
    subgraph "Tier 2 搜索链路"
        ...
        INDEX_MAINT["ToolSearchIndexMaintainer<br/>Java / MCP 工具增量维护"]
        SEARCH_SVC["ToolSearchService<br/>sanitize → 缓存 → BM25 + 语义 RRF → 排除 core/discovered/meta"]
...
### 3.3 ToolBridgeAgentToolProvider（core pinned ∪ discovered 过滤）
...
- 可见集合计算（`getToolCallbacks`）：
  ...
  - 常规：`tier1Service.getCurrentTier1Ids()` ∪ `state.discoveredToolIds()` ∪ `tool.search` 内省工具
...
- 查询流程：`ToolSearchQuerySanitizer` 规范化输入 → 缓存（`SearchResultCache` / `SessionSearchMemo`）→ FTS5 `MATCH` + BM25 排序，融合 `ToolEmbeddingIndex` 语义召回 → 排除核心 pinned / 已发现 / Meta 工具 / 权限外工具 → top-k 返回
...
- `id`：`^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)*$`；namespace 必须在自描述白名单（`memory/knowledge/notify/shell/web/file/document/cron/channel/process/tools/ui/system/git/code/workflow`）或包含动词词根（`read/write/list/...`）
- `name`：必须含中文字符
...
sequenceDiagram
    ...
    T1-->>BR: pinned 配置列表
    BR->>BR: visible = core pinned ∪ discovered ∪ meta
    BR-->>LLM: 注入 visible 工具 schema
    LLM->>BR: 直接调用 memory / file.read / status / skill.load / tool.search
...
### 4.2 LLM 发现非核心 Java / MCP 工具（search + call）
...
| 分层暴露 | 核心 pinned 常驻，其余 Java/MCP 走 `tool.search` | 保留稳定启动工具，同时验证检索链路对执行类能力的召回效果，降低默认上下文 schema 噪声 |
...
- **Agent 引擎**（`agent`）：通过 `ToolBridgeAgentToolProvider` 提供工具回调，`ReactAgentLoop` 在 state 中累积 `discoveredToolIds`（tool.search 发现）
- **MCP 协议**（`mcp`）：`McpTool` 注册到 `DynamicToolRegistry`，由 `ToolSearchIndexMaintainer` 维护到 FTS5 索引；MCP 工具通过 `tool.search` 被发现
- **Skill 系统**（`skill`）：通过 `skill.load(names=[...])` BuiltinTool 实现按需加载（1-3 个/次），返回 Skill 指南内容；`suggested_tools` 仅作为元数据，不驱动工具注入
...
| 配置键 | 默认值 | 说明 |
|--------|--------|------|
| `lifepilot.tool.tier1.pinned` | 见 application.yml | 人工固定的核心工具 ID 列表（当前：`memory` / `file.read` / `status` / `skill.load` / `tool.search`） |
