...
graph TB
    ...
    subgraph "四个内置 Skill Provider"
        TSP["TodoSkillProvider<br/>order=10 / 6 工具"]
        SSP["ScheduleSkillProvider<br/>order=20 / 6 工具"]
        HSP["HabitSkillProvider<br/>order=30 / 7 工具"]
...
监听 `ApplicationReadyEvent`，使用 `@Order(HIGHEST_PRECEDENCE)` 确保在 SkillFileWatcher 之前执行。收集所有 `BuiltinSkillProvider` Bean，按 order 升序排列，依次调用 `registerTools()` 和 `register()`。单个注册失败记录 ERROR 日志，不中断启动。

### 3.4 TodoSkillProvider（待办管理，order=10）
...
### 3.5 ScheduleSkillProvider（日程管理，order=20）
...
### 3.6 HabitSkillProvider（习惯养成，order=30）
...
内置 Skill 的配置通过 Skill 系统统一管理（`lifepilot.skills.*`），无独立配置项。四个 Provider 的注册由 `lifepilot.skills.enabled=true` 控制。MemorySkillProvider 额外依赖记忆系统的 Bean 可用性。
