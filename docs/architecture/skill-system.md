...
模块包含 11 个子包：model（数据模型）、registry（注册与搜索）、activation（激活）、markdown（Markdown 解析与热加载）、validation（三重验证管线）、generation（自扩展）、disclosure（渐进式披露工具）、config（自动配置）、event（事件）、audit（审计）、memory（记忆访问控制）。📋 规划中的子包：builtin（内置 Skill）。
...
graph TB
    ...
    subgraph "渐进式披露 disclosure"
        SDT["SkillDisclosureTool<br/>load_skill 工具（支持 skill_ids 数组）"]
        SGT["SkillGenerationTool<br/>generate_skill 工具"]
    end

    subgraph "📋 规划中：内置 builtin"
...
Skill 定义的不可变 record，包含 `id`、`name`、`description`、`version`、`source`（SkillSource）、`instructions`（注入 Agent 上下文的指令）、`suggestedTools`（建议工具列表）、`triggers`（`List<String>`，用于系统提示词中的关键词匹配）、`metadata`。紧凑构造器执行非空校验和防御性拷贝（`List.copyOf` / `Map.copyOf`）。提供 `toDiscoverySummary()` 返回 XML 格式的摘要（用于系统提示词中的 Skill 目录）。
...
原 `SkillToToolBridge` 已重构为两个独立的 BuiltinTool，实现渐进式 Skill 发现：

- `SkillDisclosureTool`：注册 `load_skill` 工具到 `DynamicToolRegistry`。Agent 调用时传入 `skill_ids` 数组（一次最多加载 3 个 Skill），从 SkillRegistry 查找并通过 SkillActivator 批量激活，返回每个 Skill 的 instructions 和 suggestedTools。Skill 不存在时返回引导消息，提示使用 `generate_skill` 创建。
...
系统提示词中包含 Skill 目录摘要（skill-catalog.st，XML 格式），Agent 根据摘要决定调用 `load_skill(skill_ids=[...])` 加载已有 Skill 或 `generate_skill` 创建新 Skill。
...
sequenceDiagram
    ...
    participant SDT as SkillDisclosureTool
    participant SR as SkillRegistry
    participant SA as SkillActivator
    participant CA as ContextAssembler
    participant Audit as SkillAuditRepository

    Note over Agent: 系统提示词包含 Skill 目录摘要（XML 格式）
    Agent->>SDT: 调用 load_skill (skill_ids=[xxx, yyy])
    SDT->>SR: find(skillId) 批量查找
    SR-->>SDT: SkillDefinition
    SDT->>SA: activate(skillId)
    SA->>SA: canActivate() 检查
    SA->>SA: metricsTracker.recordActivation()
    SA-->>SDT: SkillActivation
    SA--)Audit: 发布 Activated 事件
    SDT-->>Agent: 返回 instructions + suggestedTools
...
| 工具桥接 | load_skill + generate_skill 两个 BuiltinTool | 渐进式披露：系统提示词包含 Skill 目录摘要（XML 格式），Agent 按需调用 load_skill(skill_ids=[...]) 批量加载（最多 3 个）或 generate_skill 创建，避免注册大量独立工具 |
| 内置 Skill | 📋 规划中 | BuiltinSkillRegistrar / BuiltinSkillProvider / ProactiveSkillProvider 尚未实现 |
...
| 工具系统 (`com.lifepilot.tool`) | Skill → Tool | SkillDisclosureTool 注册 `load_skill`、SkillGenerationTool 注册 `generate_skill` 到 DynamicToolRegistry |
| Agent 引擎 (`com.lifepilot.agent`) | Agent → Skill | Agent 根据系统提示词中的 Skill 目录摘要（XML 格式），调用 load_skill(skill_ids=[...]) 按需批量激活 Skill，ContextAssembler 将 SkillActivation.instructions 注入上下文 |
| 记忆系统 (`com.lifepilot.memory`) | Skill → Memory | 📋 规划中：MemorySkillProvider 依赖 HybridRetriever 和 SemanticMemory（条件装配） |
...
| 主动推理 (`com.lifepilot.agent.proactive`) | Skill → Proactive | 📋 规划中：BuiltinSkillRegistrar 收集 ProactiveSkillProvider 的 SignalSource 和 CandidateProvider，供 ProactiveAutoConfiguration 注入主动推理管线 |
