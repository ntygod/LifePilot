...
graph TB
    subgraph "规范层 spec"
        ...
        SZM["SkillZhiweiMeta<br/>suggested_tools / tags / requires"]
    ...
    subgraph "安装 install（四来源）"
        ...
        SIS["SkillImportService<br/>.skill 压缩包导入"]
    ...
    subgraph "运行时 runtime"
        ...
        CA_B["ContextAssembler.buildSkillCatalog<br/>查表 + Gate 过滤 + 关键词打分 → top-8 扁平 markdown list"]
    end
...
### 3.1 规范层 — SkillFrontmatter / SkillZhiweiMeta / SkillRequires
...
- `SkillZhiweiMeta`（`src/main/java/com/lifepilot/skill/spec/SkillZhiweiMeta.java`）：`metadata.zhiwei` 嵌套块结构化视图——`suggestedTools / tags / requires`，缺省时返回 `empty()`。`category` 和 `priority` 字段已于 2026-05 删除（`suggested_tools` 保留但不再由运行时处理）。
...
- `metadata.zhiwei.priority` 字段已于 2026-05 删除，parser 不再解析。
...
| USER_IMPORTED | `SkillImportService`（`src/main/java/com/lifepilot/skill/install/SkillImportService.java`）| 前端 `SkillInstallDialog` 上传 `.skill` 压缩包到 `POST /api/skills/import`（含 zip slip + zip bomb 防护） |
...
**`ContextAssembler.buildSkillCatalog()`**（`src/main/java/com/lifepilot/agent/context/ContextAssembler.java:1286`）

查表驱动 catalog 构建（2026-04-25 重写）：
...
4. 从 `state.goal()` 提取关键词（中文 2-gram 滑窗 + 英文 word），按命中度打分：name 命中 +5 / description 命中 +2 / tag 命中 +1
5. 按 `(score 降, name 升)` 排序，取 top-8（`SKILL_CATALOG_MAX_ENTRIES`）
6. 渲染为扁平 markdown list 格式 `- name: description`，剩余条目数 ≥ 1 时在末尾追加一行提示引导调用 `find-skills`
7. 通过 `agent/skill-catalog.st` 模板注入到 system prompt（`<skill_catalog>...</skill_catalog>`）

为什么改扁平 markdown：原 `<category name="X"><skill name="Y"><description>...</description></skill></category>` XML 嵌套对 LLM 决策无价值（category 维度并不影响 skill 选择），还浪费 token。打分截 top-8 一方面控住 prompt 长度，另一方面把和当前任务相关的 skill 顶到 LLM 注意力前列。
...
sequenceDiagram
    ...
    Note over Agent: system prompt 含 <skill_catalog> 扁平 markdown list（top-8 by score）
