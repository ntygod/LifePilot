...
> **最后更新**：2026-03-01
> **模块编号**：Phase 6 — 模块 24
> **前置依赖**：所有已完成模块（纯基础设施模块，不依赖特定业务模块）

---

## 1. 模块定位与职责边界

部署体验模块负责 LifePilot 的打包、分发、启动和升级全流程，目标是让用户以最低门槛完成部署，无论是本地桌面还是云端 VPS。
...
| 数据目录 | `~/.lifepilot/`（本地）或 Docker Volume 挂载点，存放 SQLite 数据库、配置、日志 |
...
│   ├─ Agent 引擎 + 工具系统 + 记忆系统 + ...                   │
│   └─ SQLite + sqlite-vec（数据目录：~/.lifepilot/）           │
└─────────────────────────────────────────────────────────────┘
```
...
3. **数据目录初始化**：首次运行时创建 `~/.lifepilot/` 目录结构
