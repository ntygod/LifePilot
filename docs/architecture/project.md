...
前端 API 客户端：`src/api/project.ts`；Pinia store：`src/stores/project.ts`。
...
- `session_store.project_id` / `chat_turn_memory_snapshots.project_space_id` 均不在 DB 层加 FK，依赖应用层级联，误操作裸 SQL 可能产生孤立数据
- 未做项目级配额、审计、多用户共享语义
