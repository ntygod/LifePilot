...
- 数据文件存储在 `~/.zhiwei/` 目录，已支持本地备份、备份校验和恢复准备目录；当前不会自动覆盖恢复，正式恢复仍需用户在关闭知微后手动确认并复制数据
...
以下能力因与「本地个人助手」定位无关而移除或默认关闭：

- 代码沙箱与 Python kernel：`lifepilot.sandbox.enabled` 默认 `false`，`code` 与 kernel 工具不注册；需要时可自行开启
- git 工具：`lifepilot.meta.infra.git.enabled` 默认 `false`
- A2A 协议（Agent-to-Agent 互操作）：已整体移除
- 开发者面预置技能（代码助手 / API 调试 / 数据库查询 / GitHub 工作流 / 日志分析 / 健康检查 / 桌面自动化）：已删除，预置技能 20 → 13
- 旧主动提醒决策栈（心跳轮询 + 行为插件 + 信任阶梯 + 反馈回流）：已删除，主动性改由事件驱动的 InitiativeEngine 承担
- 桌面悬浮窗：随旧主动引擎一并移除
...
- 扩展市场依赖 ClawHub 远程索引，网络不可达时仅能使用已安装扩展
