...
> **扔一份文档进来，或者说清楚需要什么 —— 知微负责把它读懂、写出来、改到位，产物能直接用。**

和市面同类产品的差异：
- vs **通用 AI 聊天（ChatGPT / Claude / Gemini）**：它们能聊文档，但产出文件的样式保真和闭环交互弱；知微用"工具 + 本地运行时 + 双栏 artifact"做闭环
- vs **WPS AI / 微软 Copilot**：它们绑定在办公套件里；知微是独立 Agent，可编排工具链、可跨渠道、可自定义 skill
- vs **Notion AI / 飞书智能伙伴**：它们绑定在各自文档平台；知微可处理任何来源的本地/云端文档
...
**触发**：用户把 docx/xlsx/pptx/pdf 拖进对话，问内容相关问题。

**期望流**：上传 → 自动解析 → 对话里能看到文档摘要卡片 + 可展开预览 → Agent 回答时能引用到具体段落 / 单元格 / 幻灯片。
...
**触发**：用户上传现成文档，用自然语言要求修改。

**期望流**：上传 → 预览 → 下指令（"把第 2 段改成 XXX""B 列改成 C 列乘 1.1""删掉第 3 页") → 知微产出修改计划 + diff 预览 → 用户确认 → 保存为新版本 / 覆盖原档。
...
**期望流**：拖入一批文件 → 说明动作（"汇总这 5 份合同的付款条款成一张表""合并这 10 份月报成年报""把所有 docx 转 pdf"）→ 进度可见 → 产物打包下载。

### 4.5 场景 S5 — 模板填空

**触发**：用户有一份固定格式文档（合同 / 周报 / 台账 / 简历），希望填空式产出。

**期望流**：上传模板或从模板库选 → 识别变量槽 → 对话里收集变量值（或从记忆 / 结构化数据源拉）→ 渲染产物 → 下载。
