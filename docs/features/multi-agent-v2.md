# 多 Agent 协作功能说明（v2 — 四层能力模型重构）

> **模块编号**：21（Phase 6）
> **最后更新**：2026-02-27
> **版本说明**：本文档是 `multi-agent.md` 的重构版本，基于四层能力模型重新定义 Skill 和 Agent 的边界。
...
本次重构解决了 LifePilot 中 Skill 和 Agent 职责重叠的根本问题，建立清晰的四层能力模型：
...
**之前**：Skill 系统同时承担 L1（确定性工作流）和 L2（SubAgent 激活 → agentLoop.run()）两个层次的职责，`SubAgentFactory` 的执行路径完全绕过 `SkillAction`，概念混淆。

**之后**：Skill 回归纯 L1 确定性工作流定位，所有 LLM 推理委托统一由 Agent（L2）通过 HandoffTool 完成。

### 1.2 用户价值

- **更高质量的专业输出**：写作、分析、调研等任务由专门优化的 Agent 处理，每个 Agent 有独立的 System Prompt 和工具集
- **成本优化**：轻量任务使用低成本模型，复杂任务使用高能力模型（通过 `preferredProvider` 配置）
- **可扩展性**：用户可通过 YAML 文件自定义专家 Agent，支持热加载，无需重启
