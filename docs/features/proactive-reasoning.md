...
LifePilot 不只是等你提问，它将在合适的时机主动帮助你。这是 LifePilot 最核心的差异化能力之一。
...
```
[周五 14:00]
LifePilot：📝 检测到你通常这个时候准备周报。
         本周完成了 8 项待办，参加了 5 场会议。
         需要我帮你生成周报草稿吗？

[周二 08:45]  
LifePilot：📅 提醒：15 分钟后有「团队周会」（会议室A）
...
超越传统的定时任务和事件触发，将构建"记忆驱动的自动化"工作流引擎。这是 LifePilot 的原创设计。

### 2.1 传统自动化 vs 记忆驱动自动化

| 维度 | 传统自动化（如 Cron） | LifePilot 记忆驱动自动化 |
|------|----------------------|--------------------------|
...
```
场景：LifePilot 发现你最近三周每周五下午都会查看周报数据
