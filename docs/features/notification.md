...
通知系统为知微提供统一的通知投递能力。主动推理引擎、工作流引擎以及未来扩展模块均可通过 `NotificationService` 接口发送通知，支持多渠道广播（Web SSE / 企微 / 飞书 / 钉钉）、富媒体内容（文本 / Markdown / 交互式卡片 / 图片）、紧急度路由、被动队列持久化和用户通知偏好管理。

通知系统从主动推理模块解耦为独立基础设施，解决了原有架构中通知能力与 ProactiveReasoner 强耦合、被动队列重启丢失、channel 字段硬编码等问题。
...
主动推理引擎检测到用户有一个即将截止的待办，通过 `NotificationService` 发送 MEDIUM 紧急度通知。通知自动广播到用户的 Web 浏览器（SSE）和企微，内容为 LLM 生成的个性化提醒文本。
