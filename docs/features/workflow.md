...
LifePilot 工作流引擎让你通过 YAML 文件定义多步骤自动化流程，无需编写任何代码。把 YAML 文件放到指定目录，引擎自动检测并注册——即写即用。
...
保存后 LifePilot 会在 30 秒内自动检测并注册这个工作流。
...
调用 LifePilot 已注册的 Skill（内置或 YAML 自定义 Skill）：
