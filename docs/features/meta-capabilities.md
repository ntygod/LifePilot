...
元能力系统为 Agent 提供开箱即用的通用执行能力和系统自省能力。通过 22 个基础工具，Agent 可以感知环境、搜索 Web 信息、执行 Shell 命令、操控浏览器、运行代码、读写文件、与用户交互；通过 4 个自省工具，Agent 可以查询自身已注册的所有能力并按需推荐。此外，模块还提供 Skill 发现和 MCP Server 自动安装能力，帮助 Agent 动态扩展工具集。
