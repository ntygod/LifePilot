...
元能力系统为 Agent 提供开箱即用的通用执行能力和系统自省能力。内置工具覆盖 Web 信息、Shell、浏览器、代码、文件、定时任务、通知和 UI 渲染等能力；其中少量核心工具默认注入，其他内置工具和 MCP 外部工具通过 `tool.search` 发现。`status` 工具用于查询系统运行状态。
...
Agent 在执行任务时，自动使用工具完成各类操作：搜索 Web 获取最新信息、执行 Shell 命令管理系统、通过浏览器自动化完成网页操作、在沙箱中运行代码验证方案、读写文件管理用户数据。当 Agent 不确定自身能力范围时，调用 `tool.search` 发现非核心 Java / MCP 工具，或通过 `status` 查询系统状态。
