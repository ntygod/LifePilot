...
- **现状**：`document.edit` / `document.create` Agent 工具已下架，文档生成和转换改走 `doc-processor` Skill + `code` / `shell.exec`。本文件中与旧工具 schema、软约束和 prompt 暴露相关的问题已转为历史背景；若继续保留文档工作区，后续重点应放在前端 REST 工作副本是否仍有产品价值。
