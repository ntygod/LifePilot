  ...
  <div class="h-full overflow-y-auto bg-background">
    <div class="container mx-auto px-6 py-6 max-w-6xl">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-3xl font-bold text-foreground">Agent / 知识库分析</h1>
      ...
      <div class="flex items-center gap-4 mb-6">
        ...
        <div class="flex items-center gap-2 ml-auto">
          ...
          <button
            class="px-3 py-1.5 rounded-md border border-input bg-background text-sm hover:bg-accent transition-colors"
      ...
      <div v-else class="space-y-6">
        <!-- Agent 统计表格 -->
        <div v-if="filterType === 'all' || filterType === 'agent'" class="space-y-4">
          <div class="flex items-center gap-2 mb-4">
            <Bot :size="20" class="text-muted-foreground" />
            <h2 class="text-xl font-semibold text-foreground">Agent 使用情况</h2>
          </div>
          
          <div v-if="sortedAgentStats.length === 0" class="py-8">
            <EmptyState
              title="暂无 Agent 数据"
          ...
          <div v-else class="rounded-lg border border-border overflow-hidden">
            <table class="w-full">
              <thead class="bg-muted/50">
                <tr>
                  <th class="px-4 py-3 text-left text-sm font-medium text-foreground">Agent 名称</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">调用次数</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">平均响应时间</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">失败率</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">总 Token</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border">
                ...
                >
                  <td class="px-4 py-3 text-sm text-foreground">{{ stat.agentName }}</td>
                  ...
                  <td class="px-4 py-3 text-sm text-right">
                    <div class="flex items-center justify-end gap-1" :class="stat.failureRate > 0.1 ? 'text-destructive' : 'text-foreground'">
        ...
        <!-- 知识库统计表格 -->
        <div v-if="filterType === 'all' || filterType === 'kb'" class="space-y-4">
          <div class="flex items-center gap-2 mb-4">
            <BookOpen :size="20" class="text-muted-foreground" />
            <h2 class="text-xl font-semibold text-foreground">知识库检索情况</h2>
          </div>
          
          <div v-if="sortedKbStats.length === 0" class="py-8">
            <EmptyState
              title="暂无知识库数据"
          ...
          <div v-else class="rounded-lg border border-border overflow-hidden">
            <table class="w-full">
              <thead class="bg-muted/50">
                <tr>
                  <th class="px-4 py-3 text-left text-sm font-medium text-foreground">知识库名称</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">检索次数</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">命中率</th>
                  <th class="px-4 py-3 text-right text-sm font-medium text-foreground">平均检索时间</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border">
                <tr
                  ...
                  class="hover:bg-muted/30 transition-colors"
                >
                  <td class="px-4 py-3 text-sm text-foreground">{{ stat.kbName }}</td>
                  <td class="px-4 py-3 text-sm text-right text-foreground">
        ...
        <!-- 完全空状态 -->
        <div v-if="filterType === 'all' && sortedAgentStats.length === 0 && sortedKbStats.length === 0" class="py-12">
          <EmptyState
            title="暂无数据"
