...
- [x] 1. 配置属性与基础设施
  - [x] 1.1 实现 CliConfigProperties 配置属性类
  ...
  - [x] 1.2 在 application.yml 中声明所有 CLI 配置项及默认值
    - 配置前缀 `lifepilot.cli`，键名使用 kebab-case
    - _Requirements: 7.4_

- [x] 2. 响应渲染器
  - [x] 2.1 实现 ResponseRenderer（格式化输出到终端）
...
- [x] 9. Checkpoint — 确认快捷命令和对话模块编译通过
  - 确保 QuickCommand、ChatCommand 编译通过，单元测试通过，ask the user if questions arise.

- [x] 10. CLI 用户确认服务
  - [x] 10.1 实现 CliUserConfirmationService（终端用户确认服务）
...
- [x] 11. JLine 3 终端交互增强
  - [x] 11.1 实现 JLine 3 Tab 补全器（CliCompleter）
...
- [x] 13. CliShell 主循环与自动配置
  ...
  - [x] 13.2 修改 LifePilotApplication.main()，在 SpringApplication.run() 前插入 FastPathRunner.tryFastPath() 调用
    - 匹配快速路径时 System.exit(0)，否则继续正常启动
    - _Requirements: 5.3, 5.4_

  - [x] 13.3 实现 CliAutoConfiguration（Spring Boot 自动配置）
