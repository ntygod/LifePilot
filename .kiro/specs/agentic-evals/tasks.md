...
- [x] 1. 配置层与数据模型
  - [x] 1.1 实现 EvalConfigProperties 配置属性类
  ...
  - [x] 1.2 实现 BenchmarkScenario record 和 ScenarioLoadException
  ...
  - [x] 1.4 实现 DimensionScore record 和 EvalResult record
  ...
  - [x] 1.5 实现 JudgeResult record 和 ReportSummary record
...
- [x] 2. 场景加载与序列化
  ...
  - [x] 2.2 实现 ScenarioSerializer 场景序列化器
...
- [x] 3. Checkpoint - 确保场景层测试通过
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. 五维规则评估器
  - [x] 4.1 实现 DimensionEvaluator sealed interface 和五个维度评估器
  ...
  - [x] 4.4 实现 TrajectoryEvaluator 轨迹评估器
...
- [x] 5. LLM-as-a-Judge 语义评估
  - [x] 5.1 实现 LlmJudge LLM 评判器
  ...
  - [x] 5.3 写单元测试：LlmJudge 降级与重试
    - Mock LlmRouter，测试超时降级、不可解析响应重试、评分裁剪
    - _Requirements: 3.3, 3.4, 3.5_

- [x] 6. Checkpoint - 确保评估器层测试通过
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. 持久化层
  - [x] 7.1 创建 Flyway V14 迁移脚本
...
- [x] 9. Checkpoint - 确保持久化和报告层测试通过
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. EvalEngine 协调器
  - [x] 10.1 实现 EvalEngine 评估引擎
