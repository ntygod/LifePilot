...
- [-] 5. ResponseContent 扩展与 MessageConverter 适配
...
- [ ] 6. FeishuApiClient / WecomApiClient 扩展
  - [~] 6.1 `FeishuApiClient` 新增 `sendInteractiveCard(String chatId, String cardJson)` 方法
  - [~] 6.2 `FeishuApiClient` 新增 `sendImage(String chatId, String imageKey)` 方法
  - [~] 6.3 `WecomApiClient` 新增 `sendNews(String userId, String title, String description, String url, String picurl)` 方法
  - [~] 6.4 更新 `FeishuChannelAdapter.doSendResponse()`：根据 MessageConverter 结果选择 sendText / sendPost / sendInteractiveCard / sendImage
  - [~] 6.5 更新 `WecomChannelAdapter.doSendResponse()`：根据 MessageConverter 结果选择 sendText / sendMarkdown / sendNews

- [ ] 7. WorkflowStep.NotifyStep 与 StepExecutor 扩展
  - [~] 7.1 在 `WorkflowStep` sealed interface 中新增 `NotifyStep` record（id、name、targetUserId、content、contentType、urgency、dependsOn、errorStrategy）+ JsonSubTypes 注解
  - [~] 7.2 在 `StepExecutor` 中新增 `executeNotify(NotifyStep, WorkflowContext, ExpressionEngine)` 方法：解析表达式 → 构造 ResponseContent → 调用 NotificationService.send() → 写入步骤输出
  - [~] 7.3 更新 `StepExecutor.execute()` switch 表达式，新增 `NotifyStep` 分支

- [ ] 8. 通知管理 REST API
  - [~] 8.1 创建 `NotificationController`：GET `/api/notifications`（分页 + urgency 过滤）、PUT `/api/notifications/{id}/read`、PUT `/api/notifications/read-all`
  - [~] 8.2 创建 `NotificationSettingsController`：GET `/api/notification-settings`、PUT `/api/notification-settings/{typeId}`
  - [~] 8.3 创建 API 对应的 DTO record（NotificationDto、NotificationSettingDto、PagedResult 等）

- [ ] 9. 单元测试与集成测试
  - [~] 9.1 `DefaultNotificationService` 单元测试（urgency 路由、广播逻辑、用户设置过滤、单渠道失败不中断）
  - [~] 9.2 `PassiveNotificationQueue` 单元测试（持久化 enqueue/drainAll、启动加载、数据库写入失败降级）
  - [~] 9.3 `NotificationScheduler` 单元测试（定时 drain + SSE 广播、异常不中断）
  - [~] 9.4 `FeishuMessageConverter` 单元测试（ImageContent 渲染、CardContent→interactive 卡片、MarkdownContent 链接转换）
  - [~] 9.5 `WecomMessageConverter` 单元测试（ImageContent 降级、CardContent Markdown 格式）
  - [~] 9.6 `StepExecutor` NotifyStep 单元测试（表达式解析、NotificationService 调用、错误策略）
  - [~] 9.7 `NotificationController` 集成测试（分页查询、已读标记、urgency 过滤）
  - [~] 9.8 `NotificationSettingsController` 集成测试（查询设置、更新设置）
  - [~] 9.9 全量测试验证（`mvn test`）
