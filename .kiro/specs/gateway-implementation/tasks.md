...
- [x] 1. 实现认证鉴权中间件
  - [x] 1.1 创建 TrustLevel 枚举和 AuthResult record
  ...
  - [x] 1.2 创建 AuthStrategy sealed interface 和 CliAuthStrategy
...
- [x] 3. Checkpoint — 确认认证和限流中间件
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. 实现安全检查中间件
  - [x] 4.1 创建 SecurityViolation 和 SecurityCheckResult record
  ...
  - [x] 4.2 实现 PromptInjectionDetector
  ...
  - [x] 4.3 实现 SensitiveDataDetector
...
- [x] 5. 实现路由和执行中间件
  - [x] 5.1 创建 RouteDecision sealed interface
...
- [x] 6. Checkpoint — 确认路由和执行中间件
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. 实现审计中间件和持久化
  - [x] 7.1 创建 AuditEvent record
...
- [x] 8. 实现自动配置和 CLI 通道适配器
  - [x] 8.1 创建 GatewayMiddlewareAutoConfiguration
...
- [x] 9. 集成测试和最终验证
  - [x] 9.1 编写 GatewayMiddlewareAutoConfiguration 集成测试
    - 验证所有中间件 Bean 注册成功
    - 验证 `@ConditionalOnProperty` 条件激活/禁用
    - 验证依赖注入链完整
    - _Requirements: 8.1, 8.2, 8.3, 8.4_

  - [x] 9.2 编写 AuditEventRepository 集成测试
...
- [x] 10. Final checkpoint — 确认所有测试通过
  - Ensure all tests pass, ask the user if questions arise.
