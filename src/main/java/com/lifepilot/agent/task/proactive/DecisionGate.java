...
public class DecisionGate {
    ...
    public DeliveryLevel scoreToLevel(float score, ContextPacket ctx) {
        ...
        notifyThreshold = Math.max(DeliveryLevel.queueThreshold() + 0.01f, Math.min(0.99f, notifyThreshold));
        interruptThreshold = Math.max(notifyThreshold + 0.01f, Math.min(0.999f, interruptThreshold));

        DeliveryLevel level = DeliveryLevel.fromScore(score, notifyThreshold, interruptThreshold);
