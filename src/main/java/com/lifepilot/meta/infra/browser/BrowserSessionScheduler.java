 ...
 * 浏览器会话空闲清理调度器。
 *
 * <p>每 60 秒调用 {@link BrowserSessionManager#cleanupIdleSessions()} 清理
 * 超过 {@code browser.idle-timeout-seconds} 的空闲会话，防止 Chromium
 * 会话累积和资源泄漏。</p>
 *
...
public class BrowserSessionScheduler {
     ...
     * 定时清理空闲浏览器会话。
     *
     * <p>固定间隔 60 秒触发一次，清理动作本身由 BrowserSessionManager 根据
     * {@code browser.idle-timeout-seconds} 判定超时。</p>
     */
