 ...
 *
 * <p>版本格式：MAJOR.MINOR.PATCH，可选 "v" 前缀（如 "v1.2.3"）。
 * 支持 ExtensionPackage / InstalledExtension 泛化数据模型。</p>
 *
 * @author zsg
 * @since 2026-03-08
 */
public class VersionResolver {
     ...
     * 查找有可用更新的已安装扩展列表。
     *
     * <p>将 InstalledExtension.packageId 与 ExtensionPackage.id 匹配，
     * 比较版本号，返回远程版本高于本地版本的 ExtensionPackage 列表。</p>
     *
     * @param installed      已安装扩展列表
     * @param remotePackages 远程可用扩展包列表
     * @return 有可用更新的 ExtensionPackage 列表
     */
    public List<ExtensionPackage> findUpdates(List<InstalledExtension> installed,
                                              List<ExtensionPackage> remotePackages) {
        // 按 id 索引远程包，相同 id 保留版本更高的
        Map<String, ExtensionPackage> remoteMap = remotePackages.stream()
                .collect(Collectors.toMap(ExtensionPackage::id, Function.identity(), (a, b) -> {
                    return compareSemVer(a.version(), b.version()) >= 0 ? a : b;
                }));

        return installed.stream()
                .filter(ext -> {
                    var remote = remoteMap.get(ext.packageId());
                    if (remote == null) return false;
                    try {
                        return compareSemVer(remote.version(), ext.version()) > 0;
                    } catch (IllegalArgumentException e) {
                        // 版本格式不合法，跳过
                        return false;
                    }
                })
                .map(ext -> remoteMap.get(ext.packageId()))
                .toList();
    }
