...
`code` + Python 库(python-docx / openpyxl / python-pptx / pypdf)做生成与编辑;`shell_exec` + CLI(pandoc / pdftotext / wkhtmltopdf)做转换与提取。
...
| 用户表达 | 路径 |
|---|---|
| 生成 Word | `code` + python-docx |
| 生成 Excel | `code` + openpyxl |
| 生成 PPT / 幻灯片 / 演示文稿 | `code` + python-pptx |
| Markdown / HTML / DOCX / PDF 互转 | `shell_exec` + pandoc |
| PDF 文本提取 | `shell_exec` + pdftotext,或 `code` + pypdf |
| HTML → PDF | `shell_exec` + wkhtmltopdf |
...
- **结果验证**:生成后用 `file_read` 抽样确认 —— 库兼容性问题可能让 `exitCode=0` 但输出异常
