...
description: 当用户要撰写文章、报告、邮件、博客、提案或对已有文本做润色改写时使用。关键词：写文章、写报告、起草邮件、写文案、润色、改写、博客、提案、演讲稿、工作总结。从已有内容提炼摘要用 summarizer，技术文档/代码注释用 code-assistant，数据报表用 data-analyst。
version: 2.0.0
metadata:
  zhiwei:
    priority: normal
...
2. **素材收集**（按需）：`web.search` / `web.fetch` / `knowledge.search` / `file.read` / `memory(action="search")`
