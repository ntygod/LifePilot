...
1. file_read(list) 列文件清单
2. file_read(info) 拿 size，同 size 的进入下一步
3. shell_exec 算 md5（Windows: `certutil -hashfile <path> MD5`，Linux: `md5sum <path>`）
4. 同 hash 归组 → 推荐保留最早/路径最规范的
5. 用户决定 → file_manage(move) 挪多余到 trash

## 旧文件清理（按 mtime）

1. file_read(info) 收集 lastModified
2. 筛 lastModified < 当前-30 天的文件 → 列预览
3. 用户确认 → file_manage(move) 到 .trash/yyyy-MM-dd/（保留 7-30 天再清）
...
| 文件被占用 | 提示关程序后重试 |
| 用户反悔 | 按 rename-map.json 反向 file_manage(move) 回滚 |

## 可用脚本

以下脚本位于 `{skill_scripts_dir}/`，用 `shell_exec(command="python <path> <args>")` 调用：
