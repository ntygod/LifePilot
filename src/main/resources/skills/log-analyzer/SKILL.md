...
1. **定位日志**：`file.read` 列 log 目录；常见路径见参考（`/var/log` / 知微 dataDir 下 logs 等）
...
- 输出错误时间线、频率、代表性日志片段、疑似根因和下一步。
- 大量日志分析结果可落盘为报告，并返回路径。
- 引用日志必须脱敏，并保留足够定位信息。


## 失败策略

- 找不到日志文件或时间范围不明时先定位路径和窗口。
- 日志量过大时先采样、按时间/级别过滤，再扩大范围。
- 证据不足时明确列出还需要的日志或上下文。
