---
name: datastore
description: 当用户要在知微内置数据空间做集合创建、文档增删改查、按字段过滤排序、时序聚合统计（均值/求和/趋势）时使用。关键词：创建集合、记录数据、存起来、数据集合、统计趋势、聚合查询、书单、联系人、笔记。外部数据库用 database-query，记忆/知识图谱用 memory，自然语言语义提问走 knowledge.search。
version: 3.0.0
metadata:
  zhiwei:
...
文档以**自然语言富文本（content）**为主表示，直接参与语义检索。结构化字段（metadata）为副索引，支持排序、过滤、聚合。
...
**关键**：用户用自然语言提问（"那本讲外星文明的书"）时用 `knowledge.search`，按字段精确查找（"评分大于 4 的书"）时用 `datastore(action="query")`。
