...
description: 当用户要创建多步骤自动化工作流（含条件分支、循环、并行、人工审批等），用对话方式生成 YAML 定义并保存时使用。关键词：创建工作流、自动化流程、编排任务、工作流、YAML、每天检查并发送、条件分支。简单定时任务用 cron-scheduler，单步操作直接执行，代码级自动化用 code-assistant。
version: 2.0.0
metadata:
  zhiwei:
    category: automation
...
- condition / loop / parallel 完整 YAML 模式：参见 {skill_dir}/references/yaml-patterns.md
