...
通过 `ui.emit` 向前端提交组件树，用户点击后通过 signal 回调后端。判断依据：**组件不需要 signal，就不需要 A2UI**，改用 Markdown。
...
3. **先文字说明上下文**，再调用 `ui.emit` 渲染
4. **提交组件树**：每次最多 50 个组件；signal 放组件顶层，不进 properties
5. **id 唯一**：同一调用内组件 id 不能重复

## 详细参考

- 已注册组件完整目录 + ui.emit 调用样例 + Markdown vs A2UI 选择矩阵：`{skill_dir}/references/component-catalog.md`
</content>
</invoke>