...
class MessagePersistence_BugCondition_探索测试 {
    ...
    void messageId_done事件与数据库一致() {
        ...
        String assistantMessageId = historyStore.appendAssistantMessage(sessionId, assistantMessage, null, traceId);
    ...
    void 消息同步写入_方法返回时已持久化() {
        ...
        // 同步写入助手消息
        historyStore.appendAssistantMessage(sessionId, assistantMessage, null, traceId);
