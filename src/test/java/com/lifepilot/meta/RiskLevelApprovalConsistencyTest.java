...
/**
 * 风险等级审批一致性测试 — 对应 Property 3: 风险等级与审批模式一致。
 *
 * <p>验证 shell.exec 和 code.execute 的 riskLevel == HIGH 时，
...
class RiskLevelApprovalConsistencyTest {
    ...
    private ToolInput 创建空输入(String toolId) {
        return new ToolInput(toolId, Map.of(), JsonSchema.empty(), null);
    }
    ...
    void MEDIUM风险infrastructure工具_走完整策略评估_不跳过() {
        ...
        // MEDIUM 风险走策略评估 → ToolRiskPolicy defaultRiskLevel=LOW → AUTO → Passed
    ...
    private void 注册ToolRiskPolicy() {
        try {
            ...
            // shell.exec 和 code.execute 映射到 HIGH，默认 LOW
