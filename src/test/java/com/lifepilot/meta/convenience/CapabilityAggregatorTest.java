...
class CapabilityAggregatorTest {
    ...
    void aggregate_聚合各注册中心数据() {
        ...
        assertThat(summary.skills().getFirst().source()).isEqualTo("yaml");
    ...
    void aggregate_SkillSource映射正确() {
        var userSkill = createSkill("s1", new SkillSource.UserDefined("/path", null));
        var autoSkill = createSkill("s2", new SkillSource.AutoGenerated("trace-1", java.time.Instant.now(), null, false));
        var marketSkill = createSkill("s3", new SkillSource.Marketplace("pkg-1", "https://example.com", java.time.Instant.now()));

        when(skillRegistry.listAll()).thenReturn(List.of(userSkill, autoSkill, marketSkill));

        var summary = aggregator.aggregate();
        var sources = summary.skills().stream()
                .collect(java.util.stream.Collectors.toMap(CapabilityInfo::id, CapabilityInfo::source));

        assertThat(sources.get("s1")).isEqualTo("yaml");
        assertThat(sources.get("s2")).isEqualTo("auto-generated");
        assertThat(sources.get("s3")).isEqualTo("marketplace");
    }
