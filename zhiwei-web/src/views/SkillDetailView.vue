...
const isEnabled = computed(() => skill.value?.enabled !== false)
const sourceValue = computed(() => sourceLabel[skill.value?.sourceType ?? ''] ?? skill.value?.sourceType ?? '未知')
// 仅用户导入 / AI 生成的 Skill 可以直接编辑 Markdown；内置和市场来源通过 SkillController 编辑时会自动转 USER_IMPORTED
const editableMarkdown = computed(() => skill.value?.sourceType === 'USER_IMPORTED' || skill.value?.sourceType === 'AUTO_GENERATED')
...
async function saveBasicInfo() {
  ...
  try {
    await skillStore.updateSkill(skill.value.name, {
...
async function toggleSkill() {
  ...
  try {
    await skillStore.setSkillEnabled(skill.value.name, !isEnabled.value)
    uiStore.showToast('success', isEnabled.value ? '技能已停用。' : '技能已启用。')
  } catch (event: any) {
