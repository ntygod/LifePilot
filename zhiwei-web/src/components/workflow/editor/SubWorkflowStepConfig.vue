<!--
  子工作流步骤配置组件。
  配置 workflowId（目标工作流 ID）和 params（JSON 格式）。
-->
...
function updateWorkflowId(val: string | number) {
  emit('update:modelValue', { ...props.modelValue, workflowId: String(val) })
}
