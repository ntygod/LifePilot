...
interface Props {
  ...
  open: boolean
  /**
   * 嵌入模式：去掉组件自带的 fixed 抽屉壳、自身过渡、自身 header 关闭按钮，
   * 只渲染文档列表内容区。用于把它塞进 OverlayHost 这类外层容器时避免双层抽屉。
   */
  embedded?: boolean
}
...
<template>
  <!-- 嵌入模式：只渲染内容，供外层 OverlayHost 承载 -->
  <div v-if="embedded" class="doc-workspace doc-workspace--embedded">
    <header class="doc-workspace__header">
      <div class="flex items-center gap-xs">
        <span class="font-medium text-foreground">文档工作区</span>
        <span class="text-xs text-muted-foreground">{{ docs.length }} 份</span>
      </div>
      <button
        type="button"
        class="doc-workspace__refresh"
        title="刷新"
        :disabled="loading"
        @click="load"
      >
    ...
    <div class="doc-workspace__body">
      ...
      <div v-else class="doc-workspace__empty">
        当前会话还没有编辑中的文档。<br />
...
.doc-workspace__refresh:hover {
  color: var(--foreground);
  background: var(--chat-action-hover-bg);
}

.doc-workspace__refresh:disabled {
  opacity: 0.5;
  pointer-events: none;
}
