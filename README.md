...
### 🤖 Agent 引擎
- ReAct 循环架构（Thought → Action → Observation），手动控制 tool calling
...
ZhiWei 本身免费开源，但需要自备 LLM API Key（如 DeepSeek、OpenAI 等）。
...
支持 DeepSeek、OpenAI、通义千问、Ollama（本地）、智谱 GLM、文心一言等多种 LLM Provider。

### 如何自定义 Skill？

在 `~/.zhiwei/skills/` 目录下创建 Markdown 文件即可定义自定义 Skill，支持热加载。
